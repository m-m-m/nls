...
/**
 * Provides the API for the native language support (NLS). <a name="documentation"></a>
 * <h2>Native Language Support (NLS)</h2> Applications that should be used by people all over the world need <em>native
 * language support</em> (NLS). The developers task is the <em>internationalization</em> (i18n) where the application
 * has to be written in a way that the code is (mostly) independent from locale-specific informations. This is a
 * challenging task that affects many aspects like GUI-dialogs as well as all text-messages displayed to the end-user.
 * The NLS provided here only addresses the internationalization of text-messages in a way that allows
 * <em>localization</em> (l10n) to the users locale. <br>
 * <h3>The Problem</h3>
 * <p>
 ...
 * object that is called {@link net.sf.mmm.nls.NlsMessage}. For exceptions there is additional support via
 * {@link net.sf.mmm.nls.exception.NlsException}. Here is an example to clarify the idea of
 * {@link net.sf.mmm.nls.NlsMessage}: The i18n message is "Hi {name}! How are you?" and the dynamic argument is the
 ...
 * returns an {@link net.sf.mmm.nls.NlsMessage}:
 *
 * <pre>
 * package foo.bar;
 *
 * public final class NlsBundleFooBar extends {@link net.sf.mmm.nls.bundle.NlsBundle} {
 *
 *   public static final NlsBundleFooBar INSTANCE = new NlsBundleFooBar();
 *
 *   {@link net.sf.mmm.nls.NlsMessage} messageSayHi(String name) {
 *     return create("messageSayHi", "Hi {name}! How are you?", {@link net.sf.mmm.nls.argument.NlsArguments#ofName(Object) NlsArguments.ofName}(name));
 *   }
 *
 *   {@link net.sf.mmm.nls.NlsMessage} errorLoginInUse(String login) {
 *     return create("errorLoginInUse", "Sorry. The login '{name}' is already in use. Please choose a different login.", {@link net.sf.mmm.nls.argument.NlsArguments#ofName(Object) NlsArguments.ofName}(login));
 *   }
 * }
 * </pre>
 *
 ...
 * {@link net.sf.mmm.nls.NlsMessage} msg = NlsBundleFooBar.INSTANCE.messageSayHi(userName);
 * String textDefault = msg.{@link net.sf.mmm.nls.NlsMessage#getLocalizedMessage() getLocalizedMessage}());
 * String textDe = msg.{@link net.sf.mmm.nls.NlsMessage#getLocalizedMessage(java.util.Locale) getLocalizedMessage}({@link java.util.Locale#GERMAN}));
 * </pre>
 *
 * For the error message create an exception like this:
 *
 * <pre>
 * public class LoginAlreadyInUseException extends {@link net.sf.mmm.nls.exception.NlsException} {
 *
 ...
 *     super(cause, NlsBundleFooBar.INSTANCE.errorLoginInUse(login));
 *   }
 * }
 * </pre>
 *
 ...
 * See {@link net.sf.mmm.nls.NlsMessage} for further details.
 *
 ...
 * <ul>
 * <li>The {@link net.sf.mmm.nls.NlsMessage} allows to store an internationalized message together with actual arguments
 * to fill in.</li>
 * <li>The arguments can be arbitrary objects including {@link net.sf.mmm.nls.NlsObject}s that will be localized
 * recursively.</li>
 * <li>There are powerful ways to format these arguments including variable expressions for optional arguments or plural
 * forms. See {@link net.sf.mmm.nls.NlsMessage} for advanced examples.</li>
 ...
 * {@link net.sf.mmm.nls.NlsMessage#getLocalizedMessage(java.util.Locale)}.</li>
 * <li>For exceptions there is additional support via {@link net.sf.mmm.nls.exception.NlsException}.</li>
 * </ul>
 */
